...
class NHP_Options_upload extends NHP_Options{	
	...
	*/
	function render(){
		...
		echo ' <a href="javascript:void(0);" class="nhp-opts-upload button-secondary"'.$upload.' rel-id="'.$this->field['id'].'">'.__('Browse', 'MTSTHEMENAME' ).'</a>';
		echo ' <a href="javascript:void(0);" class="nhp-opts-upload-remove"'.$remove.' rel-id="'.$this->field['id'].'">'.__('Remove Upload', 'MTSTHEMENAME' ).'</a>';
	...
	*/
	function enqueue(){
		...
		wp_localize_script('nhp-opts-field-upload-js', 'nhp_upload', array(
			...
			'title' => __( 'Select Image', 'MTSTHEMENAME' ),
			'buttonText' => __( 'Select Image', 'MTSTHEMENAME' )
		));
